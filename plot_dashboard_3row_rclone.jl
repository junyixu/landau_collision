...
tag = length(ARGS) >= 1 ? ARGS[1] : "bimodal-v1"
...
const REMOTE = "mpcdf-s3://collision-operators/$tag"   # bimodal-v1 (dash) ✓
fname = replace(tag, '-' => '_')                       # bimodal_v1
csv   = "conservation_history_$(fname).csv"
raw, _ = readdlm(IOBuffer(read(`rclone cat $REMOTE/$csv`)), ',', Float64, '\n'; header=true)
