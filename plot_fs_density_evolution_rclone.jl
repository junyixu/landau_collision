...
# Cloud variant: reads snapshots from S3 via `rclone cat` (no local copy).
# Useful when particle CSV missing (e.g. crashed run). bp1/bp2 read from header.
# Run as:
#   julia --project=. plot_fs_density_evolution_rclone.jl <tag> [dt] [steps]
...
length(ARGS) >= 1 || error("usage: plot_fs_density_evolution_rclone.jl <tag> [dt] [steps]")
tag = ARGS[1]
dt  = length(ARGS) >= 2 ? parse(Float64, ARGS[2]) : 0.001
steps = length(ARGS) >= 3 ? parse.(Int, split(ARGS[3], ',')) : [0, 100, 200, 300]

# folder uses tag as-is (dash); filenames use underscore variant
const REMOTE = "mpcdf-s3://collision-operators/$tag"
fname = replace(tag, '-' => '_')
snap(s) = "fs_snapshot_$(fname)_step$(lpad(s, 4, '0')).csv"

# fetch remote file fully into memory as a String
function rget(name)
    bytes = read(`rclone cat $REMOTE/$name`)
    isempty(bytes) && error("empty from rclone: $REMOTE/$name")
    String(bytes)
end
...
function parse_text(text)
    ...
    return lines, bp1, bp2, ndof, data_start
end

function load_coeffs(lines, ndof, data_start)
    coeffs = Vector{Float64}(undef, ndof)
    for k in 1:ndof
        coeffs[k] = parse(Float64, lines[data_start - 1 + k])
    end
    coeffs
end

# Build workspace from first snapshot
lines1, bp1, bp2, ndof, _ = parse_text(rget(snap(steps[1])))
...
fields = []
for s in steps
    lines, _, _, _, ds = parse_text(rget(snap(s)))
    c = load_coeffs(lines, ndof, ds)
